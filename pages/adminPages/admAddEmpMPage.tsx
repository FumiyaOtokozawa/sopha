...
const AdmAddEmpMPage = () => {
  ...
  const validateData = (row: {
    ...
    if (!row.myoji) {
      return { isValid: false, reason: "氏名の形式が不正" };
    }
      ...
      Papa.parse<CSVRow>(text, {
        ...
        complete: async (results) => {
          try {
            const processedData = results.data
              .map((row: CSVRow) => {
                try {
                  ...
                  const nameParts = row["メンバ属性 - 氏名"].trim().split(" ");
                  let myoji: string, namae: string | null;

                  if (nameParts.length === 1) {
                    // スペースがない場合は全体を名字として扱い、名前はnull
                    myoji = nameParts[0];
                    namae = null;
                  } else {
                    // スペースがある場合は通常通り分割
                    [myoji, namae] = nameParts;
                  }

                  const joinDateStr = row["メンバ属性 - 入社日"].replace(
                    /\//g,
                    "-"
                  );
                  ...
                  if (isNaN(joinDate.getTime())) {
                    return {
                      emp_no: Number(row["メンバ属性 - 社員番号"]) || 0,
                  ...
                  console.error("行データの処理エラー:", error);
                  return {
                    emp_no: 0,
                    email: "",
                    myoji: "",
            ...
            // 結果の作成
            const resultsList = processedData
              .map((row) => ({
                name: row.email.split("@")[0],
                emp_no: row.emp_no,
                status: !row.isValid
                  ? ("error" as const)
                  : existingEmpNos.has(row.emp_no)
                  ? ("duplicate" as const)
                  : ("success" as const),
            ...
            // エラー件数を計算
            const errorCount = resultsList.filter(
              (r) => r.status === "error"
            ).length;

            setResults(resultsList);
            setSuccessMessage(
              `処理が完了しました（新規：${validData.length}件、更新：${
                existingEmpNos.size - validData.length
              }件、エラー：${errorCount}件）`
            );
          } catch (error) {
        ...
        {results.length > 0 && (
          <div className="mt-4">
            <div className="bg-[#FCFCFC19] rounded-lg shadow-md p-4 w-full max-w-[600px] mx-auto">
              <h2 className="text-lg font-bold mb-4 text-[#FCFCFC]">
                登録結果
              </h2>
